...
integer i;
reg [6:0] cnt, cnt2, cnt3, cnt4, cnt5;
reg signed [14:0] real_in [0:3], imag_in [0:3];
...
butterfly_radix4 B1(.in_real_1(real_in[0]), .in_real_2(real_in[1]), .in_real_3(real_in[2]), .in_real_4(real_in[3]),
...
ROM_64 rom(.clk(clk), .Address(cnt2[5:0]), .TF_real(twiddle_real_in), .TF_imag(twiddle_imag_in));
...
begin
    ...
    begin
       if(cnt2[5:4] == 2'b11)begin         
            real_reg1[cnt2[3:0]] <= real_out[1];
            imag_reg1[cnt2[3:0]] <= imag_out[1];
            real_reg2[cnt2[3:0]] <= real_out[2];   
            imag_reg2[cnt2[3:0]] <= imag_out[2];   
            real_reg3[cnt2[3:0]] <= real_out[3];   
            imag_reg3[cnt2[3:0]] <= imag_out[3];
        end
        case(cnt[5:4])
        2'b00:
        begin
            real_reg1[cnt[3:0]] <= {{2{in_real[12]}},in_real};
            imag_reg1[cnt[3:0]] <= {{2{in_imag[12]}},in_imag};
        end
        2'b01:
        
        begin
            real_reg2[cnt[3:0]] <= {{2{in_real[12]}},in_real}; 
            imag_reg2[cnt[3:0]] <= {{2{in_imag[12]}},in_imag}; 
        end
        2'b10:
        begin
            begin
                real_reg3[cnt[3:0]] <= {{2{in_real[12]}},in_real}; 
                imag_reg3[cnt[3:0]] <= {{2{in_imag[12]}},in_imag}; 
            end
        end
//        default:
//        for(i=0;i<16;i=i+1)
